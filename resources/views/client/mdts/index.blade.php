  ...
  <div class="card card-custom ">
    <div class="card-header">
      ...
      <div class="card-toolbar">
        ...
        <!--begin::Button-->
        <a href="{{ route('mdts.create') }}" class="btn btn-primary font-weight-bolder">
    ...
    <div class="card-body">
      ...
      <div class="">
	      <form action="{{route('client.delete-selected-mdts')}}" method="post" id="client_form">
      ...
      <table class="table table-bordered table-hover table-checkable" id="clients" style="margin-top: 13px !important">
        ...
        @php
            $mdts = \App\Models\Mdt::all();
        @endphp
    @foreach ($mdts as $mdt)
        @if($mdt->school == $school)
    ...
    <tr>
        ...
        <th>{{$mdt->name}}</th>
        <th>{{$mdt->school}}</th>
        <th>{{$mdt->ip_address}}</th>
        <th>{{$mdt->created_at}}</th>
        <th>{{$mdt->action}}</th>
    </tr>
		  ...
		  <div class="modal-dialog modal-dialog-centered" role="document">
			  <div class="modal-content">
				  <div class="modal-header">
					  ...
					  <h4 class="modal-title" id="myModalLabel">Mdt Detail</h4> </div>
...
@section('scripts')
  ...
  <script>
      ...
      function viewInfo(id) {
          ...
          $.post("{{ route('client.getMdt') }}", {_token: CSRF_TOKEN, id: id}).done(function (response) {
      ...
      function del(id){
          ...
          icon: "warning",
              ...
              if (result.value) {
                  Swal.fire(
                      "Deleted!",
                      "Your mdt has been deleted.",
                      "success"
                  );
                  var APP_URL = {!! json_encode(url('/')) !!}
                  window.location.href = APP_URL+"/client/mdt/delete/"+id;
              }
          });
      }
